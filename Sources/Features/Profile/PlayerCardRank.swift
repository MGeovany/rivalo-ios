...
/// Iridescent shine for the Holographic tier — follows device tilt.
struct CardRankHolographicEffect: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            let dx = motion.roll * w * 0.4
            let dy = motion.pitch * h * 0.4

            ZStack {
                LinearGradient(
                    colors: [
                        Color(red: 1, green: 0.45, blue: 0.75).opacity(0.28),
                        Color(red: 0.45, green: 0.85, blue: 1).opacity(0.32),
                        Color(red: 1, green: 0.92, blue: 0.45).opacity(0.24),
                        Color(red: 0.75, green: 0.45, blue: 1).opacity(0.22),
                        Color.clear,
                    ],
                    startPoint: UnitPoint(x: 0.1 + Double(motion.roll) * 0.35, y: 0.05),
                    endPoint: UnitPoint(x: 0.9 + Double(motion.roll) * 0.2, y: 0.95)
                )
                .blendMode(.screen)
                ...
                .rotationEffect(.degrees(-20 + Double(motion.roll) * 24))
                .offset(x: dx * 0.55, y: dy * 0.35)
                .blendMode(.overlay)
            }
            .opacity(0.65)
        }
